...
Approximate maximum recording duration from a single full battery charge.  A typical configuration may be 100 Hz &plusmn;8 _g_.  The [auxiliary data channels](ax3-auxiliary.md) (device temperature and light level indicator) are sampled at a lower rate in all modes.
