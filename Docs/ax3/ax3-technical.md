...
typedef struct
{
    ...
    int16_t  reserved3;                         ///< @42  +2   (Unused: originally reserved for a "Time Zone offset from UTC in minutes", 0xffff = -1 = unknown)
...
The AX3/AX6 devices are low-power devices with an internal battery.  While connected to 5V power (incuding a standard USB connection) they automatically recharge their internal battery (AX3: up to ~150mA, AX6: ~250 mA).  The charging will automatically terminate when the battery is at full capacity (reducing their power consumption while connected).  See [Battery](https://github.com/openmovementproject/openmovement/blob/master/Docs/ax3/ax3-troubleshooting.md#battery) for additional information, including notes on the charging configuration.

The devices may be configured to log during a specific interval with a start/stop time at 1-second granularity.  When disconnected and not logging (either without a recording configuration, before or after a scheduled interval, or if the battery or storage capacity is depleted) the device will automatically enter a very low-power mode.  

While recording, the recording configuration, principally the frequency and (for the AX6) whether the gyroscope is enabled, determines the maximum duration, and the device will automatically enter low power mode when a recording stops.  See [maximum recording duration](https://github.com/openmovementproject/openmovement/blob/master/Docs/ax3/ax3-faq.md#maximum-recording-duration) for which settings are likely to be limited by either the storage or battery capacity.  The configuration should be chosen based on data collection requirements rather than any concerns about power consumption.  

The AX3 does have a "low power" sensor setting for the [underlying sensor](https://github.com/openmovementproject/openmovement/blob/master/Docs/ax3/ax3-faq.md#sensor-data-characteristics), but this is not typically recommended as the data is slightly noisier (and it may not make a significant difference to the battery life).  
