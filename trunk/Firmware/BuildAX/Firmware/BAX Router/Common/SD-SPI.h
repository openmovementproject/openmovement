...
#define MILLISECDELAY   (WORD)      ((GetSystemClock()/DELAY_PRESCALER/(WORD)1000) - DELAY_OVERHEAD)
...
typedef struct
{
    ...
    //BYTE SDIO_EDS * pBuffer;          //Pointer to where the read/written bytes should be copied to/from.  May be updated between calls to the handler function.
