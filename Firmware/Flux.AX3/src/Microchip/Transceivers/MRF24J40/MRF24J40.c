// [dgj] Patched InitMRF24J40() to remove inifinite loop on unresponsive hardware -- can check global status flag 'radioPresent' (also returns FALSE to MiMAC_Init(), but MiApp_ProtocolInit() does not check the result).
char radioPresent = 0;	// [dgj] Global status flag
...
radioPresent = 1;	// [dgj] Assume ok for now
j = 0xffff;			// [dgj] Timeout iterations waiting for reset
        do
        {
if (!j--) { radioPresent = 0; break; }	// [dgj]
...
j = 0xffff;			// [dgj] Timeout iterations waiting for receive mode
        do
        {
if (!j--) { radioPresent = 0; break; }	// [dgj]
...
if (!radioPresent) { return FALSE; }	// [dgj] Return false if InitMRF24J40() appeared to fail.
