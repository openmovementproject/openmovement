...
char* bt_link_key = NULL;
char* btDeviceName = NULL;
			...
			switch (packet[0]) {
				...
				case HCI_EVENT_COMMAND_COMPLETE:
					if (COMMAND_COMPLETE_EVENT(packet, hci_read_bd_addr)){
						...
						// Copy my mac address to eeprom
						char *ptr;
						unsigned char i;
						ptr = GetBTMAC(); // Get ptr
						ReadProgram(DEVICE_MAC_ADDRESS, ptr, 6);
						for(i=0;i<6;i++){if(ptr[i]!=event_addr[i])break;}
						if(i!=6)
						{	
							// Match ram to eeprom
							WriteProgramPage_actual(DEVICE_MAC_ADDRESS,event_addr,6);
							memcpy(ptr,event_addr,6);
						}
						#endif
						#ifdef ENABLE_LOG_INFO
