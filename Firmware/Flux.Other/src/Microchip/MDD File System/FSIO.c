...
#ifdef DGJ_FIX
if ( c >= disk->maxcls + 2)		// +2 to match limit in FATfindEmptyCluster()
#endif
            if ( c >= disk->maxcls)
            {
...
#ifdef DGJ_FIX
    disk->sectorSize = MEDIA_SECTOR_SIZE;   // [dgj] Fixes formatting lockup
#endif
    FSerrno = CE_GOOD;
...
#ifdef DGJ_FIX
#ifdef FORMAT_SECTORS_PER_CLUSTER	// [dgj]
...
#ifdef DGJ_FIX  // [dgj] Fixes open-for-append on a full disk with invalid filesystem
