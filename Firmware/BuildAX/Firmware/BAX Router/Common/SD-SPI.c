// Minor changes by Karim Ladha, Dan Jackson, Copyright (C)2012-2014 Newcastle University.
...
BYTE gSDMode;
static MEDIA_INFORMATION mediaInformation = {0};
static ASYNC_IO ioInfo = {0}; //Declared global context, for fast/code efficient access
...
// OSCCONbits.COSC: 111-FRC, 000-FRC, 011-PLL48MIPS, 010-HSXT10MIPS, 001-FRCPLL?MIPS
unsigned short GetValidSPICON2SettingFromSysClock(void)
{
	unsigned short spixcon2 = 0x0020;
	switch (OSCCONbits.COSC){
	case (0b000)	:
	case (0b111)	:
	case (0b010)	: spixcon2 |= 0x001b; break; /*PLL off case - 2:1*/
	case (0b011)	:
	case (0b001)	: spixcon2 |= 0x000f; break; /*PLL on case - 5:1*/
	default : break;}
	return spixcon2;
}
#endif
...
{
    // Turn off the card
    SD_CD_TRIS = _INPUT;            //Card Detect - input
    SD_CS = 1;                     //Initialize Chip Select line
    SD_CS_TRIS = _OUTPUT;           //Card Select - output
    SD_WE_TRIS = _INPUT;            //Write Protect - input
        ...
        case ASYNC_READ_NEW_PACKET_READY:
            ...
            if(ioInfo.dwBytesRemaining != 0x00000000)
            {
            	...
            	#if defined __C30__ || defined __C32__
                {
                    ...
                    BYTE SDIO_EDS * localPointer = ioInfo.pBuffer;
...
// [dgj] If we were asked to read sector 0 over the USB (see main.c for patch for on-device access)
if (info->dwAddress == 0x0)
{
	// The start of the buffer is: number_of_bytes_requested - number_of_bytes_remaining - dwBytesRemaining - number_of_bytes_to_fetch_each_time
	unsigned long bufferStart = info->dwBytesRemaining - ioInfo.dwBytesRemaining - info->wNumBytes;
	unsigned long bufferLength = info->wNumBytes;
	unsigned char *buffer = (unsigned char *)info->pBuffer;
	
	// If this includes the last two bytes
	if (bufferStart + bufferLength >= 512 && bufferStart <= 510)
	{
		if (buffer[510 - bufferStart] == 0xAA && buffer[511 - bufferStart] == 0x55)
		{ 
			buffer[510 - bufferStart] = 0x55; buffer[511 - bufferStart] = 0xAA;
		}
	}
}
#endif
...
BYTE MDD_SDSPI_AsyncWriteTasks(ASYNC_IO* info)
{
	...
	#warning "HACKED!!!"
...
#ifdef ENABLE_SCRAMBLE
				if ((gSDScramble == 1) && (localCounter == MEDIA_SECTOR_SIZE))
				{
	                BYTE SDIO_EDS* lp;
	                localPointer += MEDIA_SECTOR_SIZE;    
					lp = localPointer;
					localCounter = MEDIA_SECTOR_SIZE;
	                do
	                {
	                    SPIBUF = *(--lp);
	                    localCounter--;
				...
				else if (gSDScramble == 2)
				{
					localCounter /= 2;					// Writing one word at a time
	                do
	                {
	                    SPIBUF = *localPointer;			// No increment means dummy data effectively only 32 Hz
	                    while(!SPISTAT_RBF);
	                    data_byte = SPIBUF; //Dummy read to clear SPISTAT_RBF
	                    SPIBUF = *(localPointer + 1);	// No increment means dummy data effectively only 32 Hz
	                    localCounter--;
	                    while(!SPISTAT_RBF);
	                    data_byte = SPIBUF; //Dummy read to clear SPISTAT_RBF
	                }while(localCounter);    
	                localPointer += MEDIA_SECTOR_SIZE;    	// Increment after
				}
				else
#endif
				{
...
BYTE MDD_SDSPI_MultipleSectorWrite(DWORD sector_addr, BYTE* buffer, int numSectors, BYTE allowWriteToZero)
{
    ...
    //Initialize structure so we write a single sector worth of data.
    info.wNumBytes = 512;
    info.dwBytesRemaining = numSectors * 512ul;
    info.pBuffer = buffer;
    info.dwAddress = sector_addr;
    info.bStateVariable = ASYNC_WRITE_QUEUED;
